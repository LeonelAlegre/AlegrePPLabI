...
int tip_imprimir(Tipos* pElemento)
{
	...
	{
		retorno=0;
		printf("\nID: %d - %s",pElemento->id,pElemento->descripcion);
	}
	return retorno;
}
 ...
 *
 */
int col_modificarArray(Tipos* array,int limite, int indice)
{
	int respuesta = -1;
	Tipos buffer;
...
*
*/
int col_buscarId(Tipos* array, int limite, int valorBuscado)
{
	int respuesta = -1;
	int i;
	if(array != NULL && limite > 0 && valorBuscado >= 0)
 ...
 *
 */
int col_bajaArray(Tipos* array,int limite, int indice)
{
	int respuesta = -1;
	if(array != NULL && limite > 0 && indice < limite && indice >= 0 && array[indice].isEmpty == 0)
	{
